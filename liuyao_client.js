...
class LiuyaoClient {
     ...
     */
    async analyzeHexagram(prompt, options = {}) {
        try {
            const requestData = {
                model: this.model,
                messages: [
            ...
            console.error('LiuyaoClient analyzeHexagram error:', error.message);
    ...
    async testConnection() {
        try {
            const testPrompt = '请简单回答：你好，太极贵人。';
            const result = await this.analyzeHexagram(testPrompt, { maxTokens: 50 });
